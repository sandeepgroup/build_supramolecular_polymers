...
def configuration_generate():
	...
	if(rotation_type==1):
		...
		for i in range(1,ip["size"]):
			if(i%2!=0):
				...
				rot = rot * orient
				#print(rot)
			input_in="python3 ../orient.py "+ tmpconfig+str(i-1)+".xyz" + " -tz " + str(ip["tz"])  + " -rz " + str(rot) 				+"  > tmpconfig"+str(i)+".xyz" 
			os.system(input_in)
	if(rotation_type==2):
		for i in range(1,ip["size"]):
	...
	'''
	with open('generated_'+str(rotation_type)+'_'+ip["label"]+'_'+str(ip["size"])+'.xyz','w') as f:
    		f.write(str(totatom)+"\n")
    		f.write("\n")
    		for file in rotated_files:
    			with open(file,"r") as fp: 
        			...
        			for atoms in range(int(natom)): 
           				...
           				for i in range(1,len(line)):
             					f.write("%-7.3f" %(float(line[i]))+" ")
           				f.write("\n")
	#print(" LOG: coordinates are  also written in " + 'generated_'+str(rotation_type)+'_'+ip["label"]+'_'+str(ip["size"])+'.xyz')
	'''
	#clean tmp files 
